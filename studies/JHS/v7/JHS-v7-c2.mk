# JHS - Jackson Heart Study
# dbGaP: phs000286
# Version: v7, Consent: c2 (DS-FDO-IRB-NPU)
#
# 5,306 African American participants from Jackson, MS metropolitan area.
# Largest single-site study of CVD in African Americans.
# ~3,600 consented for genetic research and dbGaP deposition.

DM_INPUT_DIR         := /sbgenomics/workspace/output/JHS_cleaned/JHS-v7-c2
DM_RAW_SOURCE        := /sbgenomics/project-files/PilotParentStudies/JHS/JHS-v7-c2
DM_SCHEMA_NAME       := JHS_v7_c2
DM_OUTPUT_DIR        := /sbgenomics/workspace/output/JHS/Schema_JHS_v7_c2
DM_TRANS_SPEC_DIR    := /sbgenomics/workspace/NHLBI-BDC-DMC-HV/priority_variables_transform/JHS-ingest
DM_MAPPING_SPEC      := $(DM_TRANS_SPEC_DIR)
DM_MAP_TARGET_SCHEMA := /sbgenomics/workspace/NHLBI-BDC-DMC-HM/src/bdchm/schema/bdchm.yaml
DM_MAPPING_PREFIX    := JHS
DM_MAPPING_POSTFIX   := -DS-FDO-IRB-NPU-data
